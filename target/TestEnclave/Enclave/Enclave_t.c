...
static sgx_status_t SGX_CDECL sgx_do_something_secret(void* pms)
{
	ms_do_something_secret_t* ms = SGX_CAST(ms_do_something_secret_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_do_something_secret_t));

	ms->ms_retval = do_something_secret();
...
SGX_EXTERNC const struct {
	...
	28,
	{
		{(void*)(uintptr_t)sgx_do_something_secret, 0},
